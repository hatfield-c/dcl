...
class BezierAlignmentObserver(ObserverInterface.ObserverInterface):
	...
	def EndEpisode(self):
		...
		if self.is_saved:
			control_points = torch.FloatTensor(self.drone.planner.GetControlPoints())

			state_indices = [0, episode_states.shape[0] // 3, (2 * episode_states.shape[0]) // 3, -1]
			control_states = torch.FloatTensor(episode_states[state_indices])

			horizon_states = []

			for i in range(4):
				control_point = control_points[i][[0]]
	...
	def GetEpisodeValue(self, episode_data):
		...
		position_last = episode_data[-1]["position"]
		rotation_last = episode_data[-1]["rotation"]
		velocity_last = episode_data[-1]["velocity"]
		angular_last = episode_data[-1]["angular_velocity"]

		drone_forward = Transform.RotationToDirection(rotation_last)
		drone_forward = Transform.GetUnit(drone_forward[:2])
		velocity_unit = Transform.GetUnit(velocity_last)

		scales = [ 0.33, 0.33, 0.33, 1/16 ]

		forward_alignment_reward = drone_forward[1] * scales[0]
		velocity_alignment_reward = velocity_unit[1] * scales[1]
		distance_reward = self.LinearReward(abs(position_last[0])) * scales[2]
		angular_reward = -np.sum(angular_last ** 2) * scales[3]

		reward = forward_alignment_reward + velocity_alignment_reward + distance_reward + angular_reward
		...
		if self.debug and self.client_id == 0 and self.episode_counter.GetCount() % self.episode_print_count == 0:
			...
			print("    Forward Reward      :", forward_alignment_reward)
			print("    Velocity Reward     :", velocity_alignment_reward)
			print("    Distance Reward     :", distance_reward)
			print("    Angular Reward      :", angular_reward)
			print("")
			print("    Forward Last        :", drone_forward)
			print("    Position Last       :", position_last)
			print("    Velocity Last       :", velocity_last)
			print("    Angular Last        :", angular_last)
