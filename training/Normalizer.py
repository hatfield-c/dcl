...
class Normalizer:
	def __init__(self, data, action_size):
		...
		self.domain_low = torch.min(data, dim = 0).values

		self.domain_high = torch.max(data, dim = 0).values

		self.means = torch.mean(data, dim = 0)
		self.std = torch.std(data, dim = 0) + 1e-20

		self.means_batch = self.means.view(1, -1)
		self.std_batch = self.std.view(1, -1)
