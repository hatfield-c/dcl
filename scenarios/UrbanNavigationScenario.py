...
class UrbanNavigationScenario(ScenarioInterface.ScenarioInterface):
	...
	def InstantiateEntities(self):
		...
		tall_building_1 = SimpleEntity.SimpleEntity(urdf_name = "entity_files/building_tall.urdf", position = [3, 7, 0], rotation = [0, 0, 0], is_static = True)

		self.static_objects["tall_building_1"] = tall_building_1

		long_building_1 = SimpleEntity.SimpleEntity(urdf_name = "entity_files/building_long.urdf", position = [-2, 1, 0], rotation = [0, 0, 0], is_static = True)

		self.static_objects["long_building_1"] = long_building_1

		short_building_1 = SimpleEntity.SimpleEntity(urdf_name = "entity_files/building_short.urdf", position = [-5, 0, 0], rotation = [0, 0, 0], is_static = True)

		self.static_objects["short_building_1"] = short_building_1

		self.static_objects["floor"] = SimpleEntity.SimpleEntity(urdf_name = "entity_files/20m_floor.urdf", is_static = True)
		self.entity_observer.RegisterEntities([tall_building_1, long_building_1, short_building_1])
